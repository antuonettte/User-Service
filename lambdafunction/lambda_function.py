    ...
    try:
        ...
        if http_method == 'GET':
            if path == '/user-management/user':
                ...
                return get_all_users(query_parameters)
...
def get_user_by_id(user_id):
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT id, username, email, first_name, last_name, profile_picture, bio, location, dob, phone_number, status, follower_count, following_count FROM users WHERE id = %s"
...
def delete_user_data(user_id):
    ...
    try:
        ...
        with comments_conn.cursor() as cursor:
            cursor.execute(delete_comments_sql, (user_id,))
...
def get_all_users(query_parameters):
    ...
    try:
        page = int(query_parameters.get('page', 1))  # Default to page 1
        limit = int(query_parameters.get('limit', 10))  # Default to 10 items per page
        offset = (page - 1)*limit
